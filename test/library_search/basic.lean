...
#guard_msgs in
example (n m k : Nat) : n * m - n * k = n * (m - k) :=
by std_apply?
...
#guard_msgs in
example (L _M : List (List Nat)) : List Nat := by std_apply? using L
...
-- Could be any number of results
#guard_msgs (drop info) in
example (P Q : List Nat) (_h : Nat) : List Nat := by std_apply? using P, Q
...
#guard_msgs in
lemma prime_of_prime (n : Nat) : Prime n ↔ Nat.Prime n := by
  std_exact?
